...
class VideoAttentionTargetDataset(Dataset):
	...
	def load_annotations(self):
		...
		split_shows = TRAIN_SHOWS if self.split == "train" else VAL_SHOWS if self.split == "val" else TEST_SHOWS
	
		column_names = ["path", "bbox_x_min", "bbox_y_min", "bbox_x_max", "bbox_y_max", "gaze_x", "gaze_y"]
		li = []
		for file in annotation_files:
			show, clip, fname = file.split("/")[-3:]
			if show not in split_shows:
				continue
			...
			df["show"] = show
			df["clip"] = clip
			df["path"] = df["path"].apply(lambda img_name: os.path.join(show, clip, img_name))
			df["split"] = self.split
            # Add stride group for later sampling for the training split
			if self.split == "train":
				num_stride_groups = np.ceil(len(df) / self.stride).astype(int)
				df["sgroup"] = np.repeat(np.arange(0, num_stride_groups), self.stride)[:len(df)]
			li.append(df)
		annotations = pd.concat(li, axis=0, ignore_index=True)

		if self.split == "train":
			self.group_indices = list(annotations.groupby(["show", "clip", "id", "sgroup"]).indices.values())
	...
	def __getitem__(self, index: int) -> Dict:
		if self.split == "train":
			...
			item = self.annotations.iloc[indices].sample().squeeze()
		else:
	...
	def __len__(self):
		return len(self.annotations) if self.split != "train" else len(self.group_indices)
